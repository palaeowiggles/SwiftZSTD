...
class ZSTDProcessorCommon
{
                                                    ...
                                                    Int)->Int ) throws -> Data
    {
        ...
        try dataIn.withUnsafeBytes{ pIn in
            try retVal.withUnsafeMutableBytes{ pOut in
                let rc = delegateFunction(pOut.baseAddress!, localCount, pIn.baseAddress!, dataIn.count)
                                                    ...
                                                    Int)->Int ) throws -> Data
    {
        ...
        try dataIn.withUnsafeBytes{ pIn in
            try retVal.withUnsafeMutableBytes{ pOut in
                let rc = delegateFunction(pOut.baseAddress!, Int(storedDSize), pIn.baseAddress!, dataIn.count)
