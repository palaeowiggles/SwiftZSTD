...
public func buildDictionary(fromSamples : [Data]) throws -> Data {
    ...
    try samples.withUnsafeBytes{ pSamples in
        try retVal.withUnsafeMutableBytes{ pDict in
            let actualDictSize = ZDICT_trainFromBuffer(pDict.baseAddress, localCount, pSamples.baseAddress, &sampleSizes, UInt32(Int32(sampleSizes.count)))
